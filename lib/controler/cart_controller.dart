...
class CartController extends GetxController{
  ...
  getData(int? addressId)async{
    if(Global.customer == null){
      return;
    }
    loading(true);
    cart = await ApiV2.getCart(Global.discountCode,addressId);
    if(cart == null){
      getData(addressId);
    }else{
      cartLength(cart!.cartList.length);
      if(cart!.discountCode!=null){
        discountCodeController.text = cart!.discountCode!.code;
      }
      loading(false);
    }
  }
  ...
  Future<bool> addOrUpdateCart(int product_id,int? option_id, int count,BuildContext context)async{
    ...
    loading(true);
    ApiResult result = await ApiV2.addToCart(product_id,option_id,count);
    print(result.toJson());
    if(result.succ){
      if(count > 0){
  ...
  Future<bool> deleteFromCart(int cart_id , BuildContext context)async{
    if(Global.customer == null){
      App.error_msg(context, "login_first");
      return false;
    }
    loading(true);
    ApiResult result = await ApiV2.deleteFromCart(cart_id);
    getData(null);
    return result.succ;
  }
          ...
          MyApi.discountCode(discountCodeController.text).then((value) {
            if(value!=null){
              ...
              if(value.frequency!= -1 && value.account_activation_time >= value.frequency){
                ...
                Global.discountCode = discountCodeController.text;
                Store.save_discount_code(discountCodeController.text);
